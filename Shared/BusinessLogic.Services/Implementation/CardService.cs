...
namespace BusinessLogic.Services.Implementation
{
    public class CardService : BaseService, ICardService
    {
        ...
        public async Task<CardBulkAddResponse> BulkAddAsync(CardBulkAddDto dto)
        {
            var response = new CardBulkAddResponse
            {
                TotalRequested = dto.Cards.Count
            };
            ...
            // Batch validation: Collect all unique IDs for each entity type
            var allMemberIds = dto.Cards.Where(c => c.MemberId.HasValue).Select(c => c.MemberId.Value).Distinct().ToList();
            var allVisitorIds = dto.Cards.Where(c => c.VisitorId.HasValue).Select(c => c.VisitorId.Value).Distinct().ToList();
            var allCardGroupIds = dto.Cards.Where(c => c.CardGroupId.HasValue).Select(c => c.CardGroupId.Value).Distinct().ToList();
            var allCardAccessIds = dto.Cards.SelectMany(c => c.CardAccessIds).Where(id => id.HasValue).Select(id => id.Value).Distinct().ToList();

            // Batch ownership validation
            var invalidMemberIds = allMemberIds.Any()
                ? await _repository.CheckInvalidMemberOwnershipAsync(allMemberIds, AppId)
                : new List<Guid>();
            var invalidVisitorIds = allVisitorIds.Any()
                ? await _repository.CheckInvalidVisitorOwnershipAsync(allVisitorIds, AppId)
                : new List<Guid>();
            var invalidCardGroupIds = allCardGroupIds.Any()
                ? await _repository.CheckInvalidCardGroupOwnershipAsync(allCardGroupIds, AppId)
                : new List<Guid>();

            // Batch fetch existing CardNumbers and Dmacs for duplicate checking
            var allCardNumbers = dto.Cards.Where(c => !string.IsNullOrEmpty(c.CardNumber)).Select(c => c.CardNumber).Distinct().ToList();
            var allDmacs = dto.Cards.Where(c => !string.IsNullOrEmpty(c.Dmac)).Select(c => c.Dmac).Distinct().ToList();

            var existingCards = await _repository.BaseEntityQuery()
                .Where(c =>
                    (c.CardNumber != null && allCardNumbers.Contains(c.CardNumber)) ||
                    (c.Dmac != null && allDmacs.Contains(c.Dmac)))
                .Select(c => new { c.Id, c.CardNumber, c.Dmac })
                .ToListAsync();

            var existingCardNumbers = existingCards.Select(c => c.CardNumber).Where(cn => cn != null).ToHashSet();
            var existingDmacs = existingCards.Select(c => c.Dmac).Where(d => d != null).ToHashSet();

            // Validate CardAccess IDs
            List<CardAccess> validCardAccesses = new List<CardAccess>();
            if (allCardAccessIds.Any())
            {
                validCardAccesses = await _cardAccessRepository.GetAllQueryable()
                    .Where(ca => allCardAccessIds.Contains(ca.Id))
                    .ToListAsync();
            }

            var validCardAccessIds = validCardAccesses.Select(ca => ca.Id).ToHashSet();

            // Check for duplicates within the batch itself
            var duplicateCardNumbersInBatch = allCardNumbers
                .GroupBy(cn => cn)
                .Where(g => g.Count() > 1)
                .Select(g => g.Key)
                .ToHashSet();
            var duplicateDmacsInBatch = allDmacs
                .GroupBy(d => d)
                .Where(g => g.Count() > 1)
                .Select(g => g.Key)
                .ToHashSet();
            ...
            // Process each card
            for (int i = 0; i < dto.Cards.Count; i++)
            {
                var cardDto = dto.Cards[i];
                var resultItem = new CardBulkResultItem
                {
                    Index = i,
                    Request = cardDto,
                    Success = false,
                ...
                try
                {
                    // Validate duplicate CardNumber
                    if (!string.IsNullOrEmpty(cardDto.CardNumber))
                    {
                        if (existingCardNumbers.Contains(cardDto.CardNumber))
                        {
                            resultItem.Success = false;
                            resultItem.Message = $"Card with Number {cardDto.CardNumber} already exists";
                            resultItem.ErrorCode = "DUPLICATE_CARD_NUMBER";
                            response.Results.Add(resultItem);
                            response.FailedItems.Add(resultItem);
                            response.TotalFailed++;
                            continue;
                        }
                        if (duplicateCardNumbersInBatch.Contains(cardDto.CardNumber))
                        {
                            resultItem.Success = false;
                            resultItem.Message = $"Card with Number {cardDto.CardNumber} is duplicated in the batch";
                            resultItem.ErrorCode = "DUPLICATE_CARD_NUMBER";
                            response.Results.Add(resultItem);
                            response.FailedItems.Add(resultItem);
                            response.TotalFailed++;
                    ...
                    // Validate duplicate Dmac
                    if (!string.IsNullOrEmpty(cardDto.Dmac))
                    {
                        if (existingDmacs.Contains(cardDto.Dmac))
                        {
                            resultItem.Success = false;
                            resultItem.Message = $"Card with Mac {cardDto.Dmac} already exists";
                            resultItem.ErrorCode = "DUPLICATE_DMAC";
                            response.Results.Add(resultItem);
                            response.FailedItems.Add(resultItem);
                            response.TotalFailed++;
                            continue;
                        }
                        if (duplicateDmacsInBatch.Contains(cardDto.Dmac))
                        {
                            resultItem.Success = false;
                            resultItem.Message = $"Card with Mac {cardDto.Dmac} is duplicated in the batch";
                            resultItem.ErrorCode = "DUPLICATE_DMAC";
                            response.Results.Add(resultItem);
                            response.FailedItems.Add(resultItem);
                            response.TotalFailed++;
                    ...
                    // Validate MemberId ownership
                    if (cardDto.MemberId.HasValue && invalidMemberIds.Contains(cardDto.MemberId.Value))
                    {
                        resultItem.Success = false;
                        resultItem.Message = "MemberId does not belong to this Application";
                        resultItem.ErrorCode = "INVALID_MEMBER";
                        response.Results.Add(resultItem);
                        response.FailedItems.Add(resultItem);
                        response.TotalFailed++;
                        continue;
                    ...
                    // Validate VisitorId ownership
                    if (cardDto.VisitorId.HasValue && invalidVisitorIds.Contains(cardDto.VisitorId.Value))
                    {
                        resultItem.Success = false;
                        resultItem.Message = "VisitorId does not belong to this Application";
                        resultItem.ErrorCode = "INVALID_VISITOR";
                        response.Results.Add(resultItem);
                        response.FailedItems.Add(resultItem);
                        response.TotalFailed++;
                        continue;
                    ...
                    // Validate CardGroupId ownership
                    if (cardDto.CardGroupId.HasValue && invalidCardGroupIds.Contains(cardDto.CardGroupId.Value))
                    {
                        resultItem.Success = false;
                        resultItem.Message = "CardGroupId does not belong to this Application";
                        resultItem.ErrorCode = "INVALID_CARD_GROUP";
                        response.Results.Add(resultItem);
                        response.FailedItems.Add(resultItem);
                        response.TotalFailed++;
                        continue;
                    ...
                    // Validate CardAccess IDs
                    if (cardDto.CardAccessIds.Any())
                    {
                        var invalidAccessIds = cardDto.CardAccessIds
                            .Where(id => id.HasValue && !validCardAccessIds.Contains(id.Value))
                            .Select(id => id.Value)
                            .ToList();

                        if (invalidAccessIds.Any())
                        {
                            resultItem.Success = false;
                            resultItem.Message = $"CardAccess with id {string.Join(", ", invalidAccessIds)} not found";
                            resultItem.ErrorCode = "INVALID_CARD_ACCESS";
                            response.Results.Add(resultItem);
                            response.FailedItems.Add(resultItem);
                            response.TotalFailed++;
                            continue;
                    ...
                    // Handle CardCardAccess relationships
                    if (cardDto.CardAccessIds.Any())
                    {
                        foreach (var accessId in cardDto.CardAccessIds.Where(id => id.HasValue))
                        {
                            var access = validCardAccesses.First(a => a.Id == accessId.Value);
                            entity.CardCardAccesses.Add(new CardCardAccess
                            {
                                CardId = entity.Id,
                                CardAccessId = access.Id,
                    ...
                    var result = await _repository.AddAsync(entity);
                    var cardRead = await _repository.GetByIdAsync(result.Id);

                    resultItem.Success = true;
                    resultItem.Message = "Card created successfully";
                    resultItem.Data = cardRead;
                    response.Results.Add(resultItem);
                    response.SucceededCards.Add(cardRead);
                ...
                catch (Exception ex)
                {
                    ...
                    resultItem.ErrorCode = "INTERNAL_ERROR";
                    response.Results.Add(resultItem);
                    response.FailedItems.Add(resultItem);
                    response.TotalFailed++;
                }
            ...
            // Send single MQTT message after all processing
            if (response.TotalSucceeded > 0)
            {
                _mqttQueue.Enqueue("engine/refresh/card-related", "");
            }

            // Send single audit log
            _audit.Created("Card", response.TotalSucceeded, $"Bulk created {response.TotalSucceeded} cards");
